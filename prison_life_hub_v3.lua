...
local function tp(pos)
    ...
    if pos and R then R.CFrame=CFrame.new(pos+Vector3.new(0,5,0))R.Velocity=Vector3.zero end
...
local function startFly()
    ...
    flyConn=RS.RenderStepped:Connect(function()
        ...
        if mv.Magnitude>0 then flyBV.Velocity=mv.Unit*flySpeed else flyBV.Velocity=Vector3.zero end
...
UIS.InputBegan:Connect(function(inp,gp)
    ...
    if inp.UserInputType==Enum.UserInputType.MouseButton1 and UIS:IsKeyDown(Enum.KeyCode.LeftControl)then
        ...
        local okv,mouse=pcall(function()return p:GetMouse()end)

        if okv and mouse and mouse.Hit then
...
mb(page3,"⌨️ RS-меню | F-полёт | N-noclip | Ctrl+ЛКМ-TP",Color3.fromRGB(40,40,60),function()end)
...
print("✅ Prison Life Hub v3 loaded")
